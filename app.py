...
KATEGORI_PENGELUARAN = ['Makanan', 'Transportasi', 'Hiburan', 'Belanja', 'Alokasi Dana', 'Pembayaran Utang', 'Pemberian Piutang', 'Lainnya']
KATEGORI_PEMASUKAN = ['Gaji', 'Hadiah', 'Freelance', 'Investasi', 'Penerimaan Piutang', 'Penerimaan Utang', 'Lainnya']
...
def index():
    ...
    try:
        ...
        semua_tabungan = supabase.table('tabungan').select('*').execute().data or []
        ...
        if dana_darurat_obj:
            ...
            dana_darurat_obj['terpenuhi'] = dana_darurat_obj.get('terkumpul', 0) >= dana_darurat_obj.get('target', 0) if dana_darurat_obj.get('target', 0) > 0 else False
...
def tambah_transaksi():
    ...
    if request.method == 'POST':
        try:
            ...
            # Logika alokasi dana darurat bisa ditambahkan kembali di sini jika perlu
            return redirect(url_for('index'))
        except Exception as e:
            return render_template('tambah_transaksi.html', error=str(e), kategori_pengeluaran=KATEGORI_PENGELUARAN, kategori_pemasukan=KATEGORI_PEMASUKAN, rekening=rekening_list)
...
# --- RUTE BARU UNTUK FITUR UTANG PIUTANG ---
...
def utang_piutang():
    ...
    try:
        ...
        print(f"Error fetching utang_piutang page: {e}")
    return render_template('utang_piutang.html', items=items)

@app.route('/tambah_utang_piutang', methods=['GET', 'POST'])
def tambah_utang_piutang():
    rekening_list = []
    try:
        rekening_list = supabase.table('rekening').select('id, nama_rekening').execute().data or []
    except Exception as e:
        print(f"Error fetching rekening list for utang: {e}")
    if request.method == 'POST':
        try:
            tipe = request.form['tipe']
            jumlah = float(request.form['jumlah_total'])
            deskripsi = request.form['deskripsi']
            rekening_id = int(request.form['rekening_id'])
            ...
            supabase.table('utang_piutang').insert({
                'tipe': tipe,
                'deskripsi': deskripsi,
                'pihak_terkait': request.form['pihak_terkait'],
                'jumlah_total': jumlah,
                'tanggal_mulai': tanggal_mulai,
                'tanggal_jatuh_tempo': request.form.get('tanggal_jatuh_tempo') or None
            }).execute()
            tanggal_obj = datetime.strptime(tanggal_mulai, '%Y-%m-%d').date()
            if tipe == 'Utang':
                kategori, desk_auto = 'Penerimaan Utang', f"Penerimaan dari Utang: {deskripsi}"
                tipe_auto = 'pemasukan'
            else:
                kategori, desk_auto = 'Pemberian Piutang', f"Pemberian Piutang: {deskripsi}"
                tipe_auto = 'pengeluaran'
            supabase.table('transaksi').insert({
                'deskripsi': desk_auto, 'jumlah': jumlah, 'tipe': tipe_auto,
                'kategori': kategori, 'tanggal': tanggal_obj.isoformat(),
                'bulan': tanggal_obj.month, 'tahun': tanggal_obj.year,
                'rekening_id': rekening_id
            }).execute()
            return redirect(url_for('utang_piutang'))
        except Exception as e:
            return render_template('tambah_utang_piutang.html', error=str(e), rekening=rekening_list)
    return render_template('tambah_utang_piutang.html', rekening=rekening_list)
...
def hapus_utang_piutang(id):
    try:
        ...
        print(f"Error deleting utang_piutang item: {e}")
    return redirect(url_for('utang_piutang'))

# ... (Sisa kode Class PDF dan Ekspor Anda tetap di sini) ...
class PDF(FPDF):
    def header(self): self.set_font('Arial', 'B', 12); self.cell(0, 10, 'Laporan Keuangan Pribadi', 0, 1, 'C'); self.ln(5)
    def footer(self): self.set_y(-15); self.set_font('Arial', 'I', 8); self.cell(0, 10, f'Halaman {self.page_no()}', 0, 0, 'C')
    def chapter_title(self, title): self.set_font('Arial', 'B', 14); self.cell(0, 10, title, 0, 1, 'L'); self.ln(5)
    def fancy_table(self, header, data):
        self.set_fill_color(230, 230, 230); self.set_text_color(0); self.set_draw_color(128); self.set_line_width(0.3); self.set_font('', 'B'); col_widths = [25, 105, 30, 25, 25]
        for i, h in enumerate(header): self.cell(col_widths[i], 7, h, 1, 0, 'C', 1)
        self.ln()
        self.set_font('')
        fill = False
        for row in data:
            self.cell(col_widths[0], 6, str(row.get('tanggal', '')), 'LR', 0, 'L', fill); self.cell(col_widths[1], 6, str(row.get('deskripsi', '')), 'LR', 0, 'L', fill); self.cell(col_widths[2], 6, "Rp {:,.2f}".format(float(row.get('jumlah', 0.0))), 'LR', 0, 'R', fill); self.cell(col_widths[3], 6, str(row.get('tipe', '')), 'LR', 0, 'L', fill); self.cell(col_widths[4], 6, str(row.get('kategori', '')), 'LR', 0, 'L', fill)
            self.ln(); fill = not fill
        self.cell(sum(col_widths), 0, '', 'T')
    def summary_section(self, total_pemasukan, total_pengeluaran, sisa_uang):
        self.add_page(); self.chapter_title('Ringkasan Keuangan'); self.set_font('Arial', '', 12)
        self.cell(50, 10, 'Total Pemasukan:', 0, 0); self.set_font('', 'B'); self.cell(0, 10, "Rp {:,.2f}".format(total_pemasukan), 0, 1); self.set_font('')
        self.cell(50, 10, 'Total Pengeluaran:', 0, 0); self.set_font('', 'B'); self.cell(0, 10, "Rp {:,.2f}".format(total_pengeluaran), 0, 1); self.set_font('')
        self.line(self.get_x(), self.get_y(), self.get_x() + 100, self.get_y()); self.ln(5)
        self.cell(50, 10, 'Sisa Uang:', 0, 0); self.set_font('Arial', 'B', 14); self.cell(0, 10, "Rp {:,.2f}".format(sisa_uang), 0, 1)
...
def ekspor_excel():
    try:
        ...
        if not transaksi:
            df_empty = pd.DataFrame(); output = BytesIO();
            with pd.ExcelWriter(output, engine='openpyxl') as writer: df_empty.to_excel(writer, index=False, sheet_name='Ringkasan')
            output.seek(0); return send_file(output, download_name='laporan_keuangan_kosong.xlsx', as_attachment=True)
        df = pd.DataFrame(transaksi)
        df['jumlah'] = pd.to_numeric(df['jumlah'], errors='coerce').fillna(0)
        df_pemasukan = df[df['tipe'] == 'pemasukan'].copy(); df_pengeluaran = df[df['tipe'] == 'pengeluaran'].copy()
        total_pemasukan = df_pemasukan['jumlah'].sum(); total_pengeluaran = df_pengeluaran['jumlah'].sum(); sisa_uang = total_pemasukan - total_pengeluaran
        summary_data = {'Deskripsi': ['Total Pemasukan', 'Total Pengeluaran', 'Sisa Uang'], 'Jumlah': [total_pemasukan, total_pengeluaran, sisa_uang]}; df_summary = pd.DataFrame(summary_data)
        output = BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df_summary.to_excel(writer, index=False, sheet_name='Ringkasan'); df_pemasukan.to_excel(writer, index=False, sheet_name='Pemasukan'); df_pengeluaran.to_excel(writer, index=False, sheet_name='Pengeluaran')
        output.seek(0)
        ...
        print(f"Error exporting to Excel: {e}"); return redirect(url_for('index'))
...
def ekspor_pdf():
    try:
        response = supabase.table('transaksi').select('*').order('tanggal', desc=True).execute()
        transaksi = response.data or []
        pemasukan_data = [t for t in transaksi if t['tipe'] == 'pemasukan']; pengeluaran_data = [t for t in transaksi if t['tipe'] == 'pengeluaran']
        total_pemasukan = sum(float(t.get('jumlah', 0)) for t in pemasukan_data); total_pengeluaran = sum(float(t.get('jumlah', 0)) for t in pengeluaran_data); sisa_uang = total_pemasukan - total_pengeluaran
        pdf = PDF('L', 'mm', 'A4'); header = ['Tanggal', 'Deskripsi', 'Jumlah', 'Tipe', 'Kategori']
        pdf.add_page(); pdf.chapter_title('Laporan Pemasukan'); pdf.fancy_table(header, pemasukan_data)
        pdf.add_page(); pdf.chapter_title('Laporan Pengeluaran'); pdf.fancy_table(header, pengeluaran_data)
        ...
        print(f"Error exporting to PDF: {e}"); return redirect(url_for('index'))
